-- IMS Floorplan: Unified High-Performance Drilling Telemetry Query
...
WITH latest_event AS (
  ...
  -- กรองเฉพาะ Event ที่เป็นสถานะเครื่องจริง (status หรือ alarm) หรือ event_code 204 หรือประเภท RUN/STOP/ALARM
...
SELECT
  ...
  CASE
    -- 1. เครื่องกำลังทำงาน -> สีเขียว (Status 1: RUN)
    WHEN UPPER(e.event_type) = 'RUN' THEN 1

    -- 2. เครื่องหยุดเพราะ Alarm / Error -> สีแดง (Status 3: ALARM)
    WHEN UPPER(e.event_type) IN ('ALARM', 'ERROR') AND e.event_code NOT IN ('204', '0204') THEN 3

    -- 3. เครื่อง STOP หรือเจอโค้ด 204 (รายงานระยะเวลาที่เครื่องหยุด) -> สีส้ม (Status 2: IDLE/STOP)
    WHEN UPPER(e.event_type) IN ('STOP', 'IDLE') OR e.event_code IN ('204', '0204') THEN 2

    -- 4. เครื่องกำลังเปลี่ยนหัวเจาะ -> สีฟ้า (Status 4: TOOL_CHANGE / PM)
    WHEN UPPER(e.event_type) = 'TOOL_CHANGE' THEN 4

    -- 5. สถานะอื่นๆ ที่ยังไม่ระบุ -> สีเทา (Status 5: UNDEFINE)
    ELSE 5
  END AS status,
  e.event_type,
  e.event_code,
