...
test('device IDs with spaces (real machine names like "LDI-A01") never reach flow context keys raw -- Node-RED\'s context.get/set parses string keys as property-expressions and throws on a bare space', () => {
    Object.keys(flowStore).forEach(k => delete flowStore[k]);
    calcNetRate('LDI-A01', { '1': { name: 'port_1', rx32: 1000, tx32: 500, err: 0, drop: 0, status: 1 } }, flow);
...
test('strips whitespace and punctuation Node-RED property-expressions reject', () => {
    ...
    assert.strictEqual(safeKey('LDI A01'), 'LDI_A01');
